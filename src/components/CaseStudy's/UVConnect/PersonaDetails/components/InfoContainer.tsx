...
const InfoContainer = ({ head, Age, Occupation, Status, Location, Tier, description, PainPoints, Goals, Pic }: Props) => {
    return (
        <div className='flex justify-around '>
            <div className='pt-[4rem] pb-[1rem] w-[39rem]'>
                ...
                <p className='my-[2rem] py-[1rem] border-l-[5px] border-[#9747ff] pl-[1rem] font-bold text-[1.25rem] leading-[2.5rem]'>{description}</p>
                <h2 className='mt-[1rem] font-semibold leading-[1.875rem] text-[1.125rem]'>Pain points:</h2>
                <ul className='list-disc'>
                    {PainPoints.map((point, index) => (
                        <li className='ml-[1rem] leading-[1.687rem]' key={index}>{point}</li>
                    ))}
                </ul>
                <h2 className='mt-[1rem] font-semibold leading-[1.875rem] text-[1.125rem]'>Goals</h2>
                <ul className='list-disc'>
                    {Goals.map((goal, index) => (
                        <li className='ml-[1rem] leading-[1.687rem]' key={index}>{goal}</li>
                    ))}
                </ul>
            </div>
